...
const MOCK_RESPONSES = {
  ...
  你是谁:
    "我是一个AI助手。在实际部署时，你需要配置有效的API密钥才能使用完整功能。",
  什么是AI:
    "人工智能（AI）是计算机科学的一个分支，致力于创建能够模拟人类智能行为的系统。这些系统可以学习、推理、解决问题、理解自然语言、感知环境等。\n\n注意：这是一个模拟回复。在生产环境中，你需要配置有效的API密钥来获取更准确的信息。",
  如何学习编程:
    "学习编程是一个循序渐进的过程。以下是一些建议：\n\n1. 选择一门入门语言（如Python或JavaScript）\n2. 学习基础概念（变量、条件语句、循环、函数等）\n3. 实践编码，从小项目开始\n4. 参与社区，向他人学习\n5. 不断挑战自己，尝试更复杂的项目\n\n注意：这是一个模拟回复。配置API密钥后可以获得更详细的指导。",
};
 ...
 */
export function getMockResponse(prompt, modelName = "DeepSeek") {
  // 查找匹配的回复
  ...
  // 默认回复（与当前“服务端代理 + .env”方案一致）
  return (
    `这是一个模拟回复。你的问题是："${prompt}"\n\n` +
    `要使用真实的 ${modelName} 回复，请按以下步骤操作：\n` +
    `1. 在项目根目录创建/编辑 .env 文件\n` +
    `2. 配置 DEEPSEEK_API_KEY=你的密钥（仅服务端使用）\n` +
    `3. 启动服务端代理：npm run dev:server\n` +
    `4. 启动前端：npm run dev\n\n` +
    `提示：也可以设置 VITE_USE_MOCK=true 强制使用 Mock 模式。`
  );
}
...
export function buildSystemPrompt(customPrompt) {
  const defaultPrompt =
    "你是一个有用的AI助手，帮助用户回答问题。你会记住之前的对话内容，并基于上下文给出更准确的回答。";
